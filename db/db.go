...
type DBFn func(*sqlite.Conn) error

func New(ctx context.Context, filename string, migrations []string) (*DB, error) {
	if filename == "" {
...
func (db *DB) Reset(ctx context.Context, shouldClear bool) (err error) {
	...
	if err != nil {
		return fmt.Errorf("could not open read pool: %w", err)
	}
...
func (db *DB) Write(ctx context.Context, fn DBFn) (err error) {
	conn, err := db.writePool.Take(ctx)
	if err != nil {
		return fmt.Errorf("failed to take write connection: %w", err)
	}
...
func (db *DB) Read(ctx context.Context, fn DBFn) (err error) {
